...
| # | 检查项 | 操作 | 通过标准 |
|---|--------|------|----------|
| 2.1 | 迁移脚本齐全 | 检查 `migrations/` 至最新版本 | 当前最高 `0029_huawei_legacy_accounts_native_sync_mode`；当前仓库未包含 `0021` Notification 迁移，按实际文件顺序执行 |
...
| 11.8 | 通知通道 | 企业微信/飞书/邮件/Webhook | 测试发送成功，失败有重试和记录 |
