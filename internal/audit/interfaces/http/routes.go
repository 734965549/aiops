...
const auditAuthResource = "audits"

// Registrar 注册 Audit 路由到 /api/audits。
...
func (r *Registrar) RegisterRoutes(groups server.RouteGroups) {
	authed := groups.Authed.Group("/audits")
	if r.authorizer == nil {
		return
	}
	authz := authorizationMiddlewareAdapter{authorizer: r.authorizer}
	authed.GET("", middleware.AuthorizeStatic(authz, auditAuthResource, "read"), r.handler.ListAudits)
}
